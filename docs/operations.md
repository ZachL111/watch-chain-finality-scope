...
`watch-chain-finality-scope` is centered on blockchain tooling work in C. The technical target is: Implement a C blockchain tooling project for finality protocol validation, using framed sample traffic and bounds and ordering tests.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 5.
